...
- **Anonymous pageview counting on the public site** via [GoatCounter](https://www.goatcounter.com/) — so we can finally tell whether anyone's using Standby. Deliberately fenced: only runs when a site code is baked in at build time (the GitHub Pages deploy sets it; Docker/self-host builds and local dev never do), respects Do Not Track, skips localhost, sets no cookies, and transmits *only* the path viewed — never any show data, which still lives only in the browser's IndexedDB. Per-route counting via React Router so we can see which features get used. New `src/lib/analytics.ts` (the single network-touching module in the app), `.env.example` documenting the `VITE_GOATCOUNTER_CODE` var, and an honest rewrite of the README's privacy claims to match. Self-hosters get zero analytics with no action required.
