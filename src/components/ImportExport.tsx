...
/**
 * JSON backup / restore for the entire show. Export downloads a single
 * `.standby.json` snapshot of every entity under the current production;
 * import loads any `.standby.json` (including ones exported from a
 * different device) as a brand-new production.
 *
 * Sample-show loading was previously in this component but is now a
 * separate <SampleShowPicker /> shown on the Production empty state.
 * Keeping these surfaces separate avoids cluttering the Backup &
 * storage page with first-run options.
 */
